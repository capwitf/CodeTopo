...
The Mermaid graph is interactive: click a node to highlight the selected node and its directly related upstream and downstream calls. Click the blank area to clear the highlight, or use the reset button to restore the default view.
...
- Support multiple model providers: DeepSeek, OpenAI, Kimi, Anthropic, GLM, MiniMax, and OpenAI-Compatible
...
```bash
git clone https://github.com/capwitf/CodeTopo.git
cd CodeTopo
```
...
If you want to report an issue, discuss collaboration, or share feedback about CodeTopo, contact: [capwitf@outlook.com](mailto:capwitf@outlook.com)

## 中文

### 项目说明

CodeTopo 是一个本地运行的代码结构分析工具。选择项目目录和目标源码文件后，后端会构建跨文件调用图，并结合大模型生成针对目标文件的分析说明。

上面的 Mermaid 图支持交互：点击节点会高亮当前节点以及与它直接相关的上下游调用关系，点击空白区域可取消高亮，点击重置按钮可恢复默认视图。
...
- 支持多种模型提供商：DeepSeek、OpenAI、Kimi、Anthropic、GLM、MiniMax、OpenAI-Compatible
...
- 项目使用 Tree-sitter 进行静态解析，使用大模型生成解释性分析
- `.venv`、缓存目录和编辑器配置不建议提交到仓库

### 联系方式

如果你想反馈问题、交流合作，或对 CodeTopo 提出建议，欢迎联系：[capwitf@outlook.com](mailto:capwitf@outlook.com)
